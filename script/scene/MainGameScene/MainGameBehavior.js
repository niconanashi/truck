...
var MainGameBehavior = /** @class */ (function (_super) {
    ...
    MainGameBehavior.prototype.onMessageEvent = function (event) {
        ...
        if (g.game.isActiveInstance())
            this._onActiveInstanceEvent(event);
    };
    /**
     * 全てのインスタンスで拾うイベント
     */
    MainGameBehavior.prototype._onAllInstanceEvent = function (event) {
        ...
        switch (messageType) {
            ...
            case MessageEventType_1.MessageEventType.changeUserTouchState:
                ...
                if ((canPlaySE == null || (!!canPlaySE && canPlaySE)) &&
            ...
            case MessageEventType_1.MessageEventType.respawnSnake:
                if (this.stateManager.playerList[event.player.id].state !== StateManager_1.PlayerState.dead)
                    break; // 重複リスポーンを防ぐ
            ...
            case MessageEventType_1.MessageEventType.respawnBroadcasterAngelSnake:
                if (this.stateManager.playerList[this.stateManager.broadcaster.id].state !== StateManager_1.PlayerState.dead)
                    break; // 重複リスポーンを防ぐ
