...
var MainGameScene = /** @class */ (function (_super) {
    ...
    MainGameScene.prototype.init = function (playerInitLayoutList) {
        ...
        this._playBGM();
        this._setSE();
        if (!!this.stateManager.sessionParameter.config.debug &&
    ...
    MainGameScene.prototype.receivePlayersInConflict = function (playersInConflict) {
        ...
        playersInConflict.forEach(function (info) {
            ...
            if (!!_this.stateManager.playerList[deadPlayerId] &&
                !!_this.stateManager.playerList[deadPlayerId].snake &&
                _this.stateManager.playerList[deadPlayerId].state === StateManager_1.PlayerState.playing) {
                ++_this.stateManager.playerList[killerPlayerId].killCount;
                // SE再生
                if (deadPlayerId === g.game.selfId ||
                    killerPlayerId === g.game.selfId ||
                    (_this._isAudience(g.game.selfId) &&
                        (deadPlayerId === _this.stateManager.broadcaster.id ||
                            killerPlayerId === _this.stateManager.broadcaster.id))) {
                ...
                // ユーザー名ラベルをhide
                _this.stateManager.userNameLabels[deadPlayerId].hide();
                // ポップ、ダッシュゲージをhide
                if (deadPlayerId === g.game.selfId) {
    ...
    MainGameScene.prototype.snakeDestructionProcedure = function (deadPlayerId) {
        if (!!this.stateManager.playerList[deadPlayerId] &&
            !!this.stateManager.playerList[deadPlayerId].snake &&
            this.stateManager.playerList[deadPlayerId].state === StateManager_1.PlayerState.staging) {
            ...
            if (deadPlayerId === g.game.selfId) {
                if (this.stateManager.isBroadcaster) {
                    // 放送者リスポーンボタン表示
                    if (this.stateManager.playerList[deadPlayerId].respawnTimes > 0)
                        this._createRespawnView();
                    else
                        this._createAngelSnakeView();
                }
                else {
                    // 視聴者リスポーンボタン表示
                    if (this.stateManager.playerList[deadPlayerId].respawnTimes > 0)
    ...
    MainGameScene.prototype.switchGoldenSnake = function (ownerId, stolenPlayerId) {
        var _this = this;
        if (!this.stateManager.playerList[ownerId].snake)
            return;
        if (stolenPlayerId != null) {
            var stolenSnake_1 = this.stateManager.playerList[stolenPlayerId].snake;
            stolenSnake_1.head.body._surface =
                g.SurfaceUtil.asSurface(this.assets["snake" + stolenSnake_1.snakeType + "_head_alive"]);
            stolenSnake_1.head.body.invalidate();
            stolenSnake_1.segments.forEach(function (seg) {
                if (seg.type === Snake_1.SnakeSegmentType.Jewel)
                    return;
        ...
        jewelOwnerSnake.head.body._surface =
            g.SurfaceUtil.asSurface(this.assets["snake" + jewelOwnerSnake.snakeType + "_head_gold"]);
    ...
    MainGameScene.prototype.rewriteRanking = function (rankingAccountData) {
        ...
        rankingAccountData.forEach(function (account, i) {
            if (broadcasterRank !== -1)
                return;
            if (account.id === _this.stateManager.broadcaster.id)
                broadcasterRank = i;
        });
    ...
    MainGameScene.prototype.rewriteTime = function (remainTime) {
        if (!this.stateManager.sessionParameter.config.time.isTimeBased)
            return;
    ...
    MainGameScene.prototype.showFinishView = function () {
        ...
        this.userFollowingLayer.append(countDown);
        this.timeline.create(countDown).fadeIn(450, akashic_timeline_1.Easing.easeInExpo);
        // リスポーンボタンが表示されていたら消す
    ...
    MainGameScene.prototype.showBroadcasterDisplayViewing = function () {
        ...
        this.timeline.create(panel)
            .fadeIn(450, akashic_timeline_1.Easing.easeInExpo)
            .wait(2000)
            .fadeOut(450)
            .call(function () {
            if (!panel.destroyed()) {
    ...
    MainGameScene.prototype._updateScene = function () {
        ...
        Object.keys(this.stateManager.playerList).forEach(function (playerId) {
            if (!_this.stateManager.playerList[playerId] ||
                !_this.stateManager.playerList[playerId].camera ||
                !_this.stateManager.playerList[playerId].snake)
                return;
            ...
            if (StateRoleChecker_1.checkStateRole(_this.stateManager.playerList[playerId].state, StateRoleChecker_1.StateRoleType.CanMoveType)) {
                ...
                _this._manageSnakeHeadOnServerInstance(playerId, snake, state);
                // スネークの移動
    ...
    MainGameScene.prototype._updateSnake = function (playerId, snake, state) {
        var deg = ((this.stateManager.playerList[playerId].uiState.direction /
            this.stateManager.sessionParameter.config.userInput.radianFineness) * 360 + 90) % 360;
        var field = { width: this.stage.nowWidth, height: this.stage.nowHeight };
        snake.update({ angle: deg, state: state, field: field });
    };
    MainGameScene.prototype._updateDeadSnakeCamera = function () {
        var _this = this;
        Object.keys(this.stateManager.playerList).forEach(function (playerId) {
            if (playerId !== _this.stateManager.broadcaster.id &&
                _this.stateManager.playerList[playerId].state === StateManager_1.PlayerState.dead) {
                var playerCamera = _this.stateManager.playerList[playerId].camera;
                if (!playerCamera) {
                    console.log("camera not found! id:", playerId);
                    return;
    ...
    MainGameScene.prototype._updateLengthCount = function (targetPlayerId) {
        if (!this.stateManager.playerList[targetPlayerId].snake ||
            this.stateManager.playerList[targetPlayerId].state === StateManager_1.PlayerState.dead)
            return;
        ...
        if (score > this.localParameter.preLengthCount &&
    ...
    MainGameScene.prototype._updateMiniMap = function (targetPlayerId) {
        ...
        if (nowOwnerId != null) {
            // お宝所有者がいる場合
            if (!StateRoleChecker_1.checkStateRole(this.stateManager.playerList[nowOwnerId].state, StateRoleChecker_1.StateRoleType.CanDropType))
                return;
            var segments = this.stateManager.playerList[nowOwnerId].snake.segments;
            var jewel = segments[segments.length - 1];
            if (!jewel || jewel.type !== Snake_1.SnakeSegmentType.Jewel)
                return;
            jewelCommonOffset = {
    ...
    MainGameScene.prototype._updateDashGauge = function () {
        if (!this.localParameter.dashingGaugeBar.visible())
            return;
        if (this.localParameter.dashingGauge >= this.stateManager.sessionParameter.config.snake.dashingTime * g.game.fps &&
    ...
    MainGameScene.prototype._setInterval = function () {
        ...
        this.setInterval(function () {
            // Foodを撒く
            if (Object.keys(_this.stateManager.playerList)
                ...
                _this.stateManager.foodList.length <= _this.stateManager.maxFoodListLength) {
                // 生き残ってるSnakeがいれば
    ...
    MainGameScene.prototype._createTouchArea = function () {
        ...
        this.touchArea.onPointDown.add(function (event) {
            if (!_this.stateManager.playerList[g.game.selfId] ||
                _this.stateManager.playerList[g.game.selfId].preventType === MessageEventType_1.PreventType.TouchState ||
                !StateRoleChecker_1.checkStateRole(_this.stateManager.playerList[g.game.selfId].state, StateRoleChecker_1.StateRoleType.CanOperateType))
                return;
        ...
        this.touchArea.onPointMove.add(function (event) {
            if (!_this.stateManager.playerList[g.game.selfId] ||
                _this.stateManager.playerList[g.game.selfId].uiState.state === UserTouchState_1.UserTouchState.onDoubleTap ||
                _this.stateManager.playerList[g.game.selfId].uiState.state === UserTouchState_1.UserTouchState.onHold ||
                !StateRoleChecker_1.checkStateRole(_this.stateManager.playerList[g.game.selfId].state, StateRoleChecker_1.StateRoleType.CanOperateType)) {
                return;
            }
            if (_this.stateManager.playerList[g.game.selfId].preventType === MessageEventType_1.PreventType.TouchState)
                return;
            var userInput = _this.stateManager.sessionParameter.config.userInput;
            var degAndNorm = calculateRadFromPoint(event.startDelta);
    ...
    MainGameScene.prototype._addNotice = function (newNotice) {
        ...
        if (this.localParameter.noticeList.length === 8) {
            ...
            if (!outdatedNotice.destroyed())
                outdatedNotice.destroy();
        }
    ...
    MainGameScene.prototype._createTmpSnake = function (playerId, layoutData) {
        var words = utils_1.stringToArray(layoutData.name)
    ...
    MainGameScene.prototype._checkStopDashing = function () {
        ...
        if (this.localParameter.dashingGauge <= 0 ||
        ...
        return (!!playerId &&
            (!this.stateManager.playerList[playerId] ||
                (playerId !== this.stateManager.broadcaster.id &&
                    !!this.stateManager.playerList[playerId] &&
                    StateRoleChecker_1.checkStateRole(this.stateManager.playerList[playerId].state, StateRoleChecker_1.StateRoleType.IsAudienceType))));
    };
    ...
    MainGameScene.prototype._dropSnakeBody = function (deadPlayerId) {
        ...
        if (!!this.stateManager.playerList[deadPlayerId] &&
            !!this.stateManager.playerList[deadPlayerId].snake &&
            StateRoleChecker_1.checkStateRole(this.stateManager.playerList[deadPlayerId].state, StateRoleChecker_1.StateRoleType.CanDropType)) {
            var deadSnake = this.stateManager.playerList[deadPlayerId].snake;
            // 節をエサとして落とす
