...
var Snake = /** @class */ (function (_super) {
    ...
    function Snake(param) {
        ...
        _this.tweens = {};
        if (param.rebornEffect)
            _this._initEffectLayer();
        _this._initLayer();
    ...
    Snake.prototype.blinking = function () {
        var _this = this;
        if (this.tweens[MessageEventType_1.AnimationType.Blinking] != null)
            return;
        this.tweens[MessageEventType_1.AnimationType.Blinking] = this.timeline.create(this, { loop: true })
            .wait(100)
            .call(function () { return _this.modifyOpacity(1.0); })
            .wait(100)
            .call(function () { return _this.modifyOpacity(0.5); });
    };
    /**
     * ダッシュエフェクト
     */
    Snake.prototype.dashing = function (state) {
        if (state === UserTouchState_1.UserTouchState.onDoubleTap) {
            if (this.head.dashEffect.visible())
                return;
    ...
    Snake.prototype.explosion = function () {
        ...
        var createEffect = function (x, y) {
            var camera = g.game.focusingCamera;
            if (!camera)
                return;
            var margin = 50;
            if (x < camera.x - margin ||
    ...
    Snake.prototype.removeTween = function (animationType) {
        if (this.tweens[animationType] == null)
            return;
    ...
    Snake.prototype._initRebornEffect = function (param) {
        ...
        var portalFrames = prevFrame.concat(centerFrame, postFrame);
        // 演出の初期化
    ...
    Snake.prototype._checkFieldBoundary = function (currentHeadPositionX, currentHeadPositionY, rad, fieldWidth, vecScale) {
        ...
        if (Math.pow(this.head.x, 2) + Math.pow(this.head.y, 2) >= Math.pow(limitRadius, 2)) {
            ...
            if (this.rotateState === SnakeRotateState.noRotate) {
                ...
                if (check > 0)
                    this.rotateState = SnakeRotateState.onClockwise;
                else
                    this.rotateState = SnakeRotateState.onCounterClockwise;
            }
    ...
    Snake.prototype._appendNewPositionHistory = function (param) {
        var currentHeadPositionX = this.head.x;
        var currentHeadPositionY = this.head.y;
        var currentHeadAngle = this.angle;
        // 先頭に新しい座標をつける
        var appendPositionHistory = [];
        // ダッシュ中は補完フレームを増やす
        ...
        if (param.state === UserTouchState_1.UserTouchState.onDoubleTap
            ...
            for (var i = 1; i < this._internalVecMaxScaleByBase; i++) { // _internalVecMaxScaleByBase - 1回だけ追加する
                var intervalElement = {
        ...
        if (this.positionHistory.length > maxHistoryLenth)
            this.positionHistory.length = maxHistoryLenth;
    };
    ...
    Snake.prototype._addKnot = function (food) {
        ...
        this.segments.forEach(function (seg) {
            if (seg.type !== SnakeSegmentType.Knot)
                return;
            if (!seg.destroyed()) {
        ...
        tmpSegments.forEach(function (seg) {
            if (_this.segments.length - (_this.haveJewel ? 1 : 0) >= _this._snakeMaxKnotLength &&
...
var SnakeSegment = /** @class */ (function (_super) {
    ...
    SnakeSegment.prototype.render = function (renderer, camera) {
        if (camera) {
            ...
            if (globalOffset.x >= cam.x - margin &&
