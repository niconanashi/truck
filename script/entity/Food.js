    ...
    Food.prototype._init = function () {
        ...
        this.food.render = function (renderer, camera) {
            if (camera) {
                ...
                var margin = 50;
                if (!foodOffset)
                    foodOffset = entityUtils_1.localToGlobal(_this);
                if (foodOffset.x >= cam.x - margin &&
        ...
        var createLabel = function () {
            ...
            var margin = 50;
            if (camera) {
                if (_this.x >= camera.x - margin &&
                    _this.x <= camera.x + g.game.width + margin &&
                    _this.y >= camera.y - margin &&
                    _this.y <= camera.y + g.game.height + margin) {
        ...
        this.root.scene.setTimeout(function () {
            if (!_this.root.scene || _this.root.destroyed())
                return; // 消費された場合
