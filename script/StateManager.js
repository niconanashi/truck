...
var StateManager = /** @class */ (function () {
    ...
    StateManager.prototype.receiveJoinRequest = function (player, user) {
        if ((g.game.age - this.startTime) > this.sessionParameter.entrySec * g.game.fps)
            return;
        if (this.applicantList.some(function (p) { return p.player.id === player.id; })) {
            return;
        }
    ...
    StateManager.prototype.startLottery = function () {
        ...
        if (!!this.sessionParameter.config.debug &&
        ...
        var winners = lottery_1.weightedLottery(this.applicantList, Math.min(this.applicantList.length, this.sessionParameter.numPlayers), this.sessionParameter.premiumWeight, this.randomGenerator, function (userData) { return userData.user.isPremium; });
        var playerList = {};
    ...
    StateManager.prototype.setCameraAndSnake = function (playerId, snake) {
        ...
        if (playerId === g.game.selfId)
            g.game.focusingCamera = this.playerList[playerId].camera;
        // 最初は無敵状態なので透過させる
    ...
    StateManager.prototype.applyTouchState = function (playerId, state, direction) {
        ...
        if (direction != null)
            this.playerList[playerId].uiState.direction = direction;
        if (!!this.playerList[playerId].snake &&
            state === UserTouchState_1.UserTouchState.onPoint &&
            direction != null) {
            ...
            if (this.playerList[playerId].snake.rotateState !== Snake_1.SnakeRotateState.noRotate) {
                ...
                if (check > 0)
                    snake.rotateState = Snake_1.SnakeRotateState.onClockwise;
                else
                    snake.rotateState = Snake_1.SnakeRotateState.onCounterClockwise;
            }
        }
        if (!!this.playerList[playerId].snake)
            this.playerList[playerId].snake.dashing(state);
    };
    /**
     * サーバインスタンスのみ実行
     * 無敵状態解除シーケンスを開始する
    ...
    StateManager.prototype.endInvincibleTime = function (scope, playerId) {
        ...
        if (scope === MessageEventType_1.ScopeType.All)
            this.setCountDown();
        // 無敵状態解除通知
        this._currentSetTimeout(function () {
            var message = {
    ...
    StateManager.prototype.applyPlayingStateForAllSnakes = function () {
        ...
        Object.keys(this.playerList).forEach(function (playerId) {
            if (!_this.playerList[playerId].snake)
                return;
    ...
    StateManager.prototype.checkSnakeCollision = function () {
        ...
        Object.keys(this.playerList).forEach(function (playerId) {
            Object.keys(_this.playerList).forEach(function (enemyId) {
                ...
                if (enemyId !== playerId &&
                    StateRoleChecker_1.checkStateRole(playerState, StateRoleChecker_1.StateRoleType.CanCollideType) &&
                    StateRoleChecker_1.checkStateRole(enemyState, StateRoleChecker_1.StateRoleType.CanCollideType)) {
    ...
    StateManager.prototype.checkEatenFoods = function (fieldRadius) {
        ...
        this.foodList.forEach(function (shownFood, foodIndex) {
            ...
            Object.keys(_this.playerList).forEach(function (playerId) {
                ...
                if (!shownFood ||
                    !playerSnake ||
                    !StateRoleChecker_1.checkStateRole(_this.playerList[playerId].state, StateRoleChecker_1.StateRoleType.CanDropType))
                    return;
                var foodArea = {
            ...
            if (!isEaten)
                noEatenFoodIndexList.push(foodIndex);
        });
        if (this.waitingFoodList.length !== 0 ||
    ...
    StateManager.prototype.updateRanking = function () {
        ...
        this.topPlayerList.forEach(function (player, i) {
            if (!isSame || !player)
                return;
            if (player.player.id !== rankingList[i].player.id)
                isSame = false;
        });
        // 変更があった場合のみ通知する
        if (!isSame || this.topPlayerList.length === 0) {
            ...
            this.topPlayerList.forEach(function (player) {
                ...
                if (!_this.playerList[player.player.id].snake ||
                    playerState !== PlayerState.playing)
                    return;
    ...
    StateManager.prototype.applyEatenFoods = function (eatenFoodInfo, noEatenFoodIndexList, fieldRadius) {
        ...
        eatenFoodInfo.forEach(function (info) {
            if (!_this.foodList[info.eatenIndex].root.destroyed())
                _this.foodList[info.eatenIndex].destroy();
            if (!_this.playerList[info.eaterId].snake)
                return;
        ...
        noEatenFoodIndexList.forEach(function (noEatenIndex) {
            ...
            if (Math.pow(food.x, 2) + Math.pow(food.y, 2) <= Math.pow(fieldRadius + 100, 2)) {
                ...
                if (!_this.foodList[noEatenIndex].root.destroyed())
                    _this.foodList[noEatenIndex].destroy();
            }
        });
        this.waitingFoodList.forEach(function (food) { return newFoodList.push(food); });
        this.waitingFoodList = [];
    ...
    StateManager.prototype.checkEatenJewel = function () {
        ...
        if (nowOwnerId != null) {
            // お宝所有者がいる場合
            if (!StateRoleChecker_1.checkStateRole(this.playerList[nowOwnerId].state, StateRoleChecker_1.StateRoleType.CanDropType))
                return;
    ...
    StateManager.prototype.checkJewelOutsideField = function (fieldRadius) {
        if (this.jewelData.ownerId != null)
            return;
    ...
    StateManager.prototype.applyEatenJewel = function (ownerId) {
        if (!this.playerList[ownerId].snake)
            return;
        var stolenPlayerId = this.jewelData.ownerId; // お宝を盗まれたプレイヤーのid
        if (stolenPlayerId != null)
            this.playerList[stolenPlayerId].snake.removeJewel();
    ...
    StateManager.prototype.setRespawnSnake = function (playerId, snakeLayer, nowRadius) {
        ...
        var _a = this._generateNewSnakeLayout(playerId, snakeLayer, true, nowRadius), snake = _a[0], direction = _a[1];
        this.playerList[playerId].snake = snake;
    ...
    StateManager.prototype.setBroadcasterAngelSnake = function (snakeLayer, nowRadius) {
        var _a = this._generateNewSnakeLayout(this.broadcaster.id, snakeLayer, false, nowRadius), snake = _a[0], direction = _a[1];
    ...
    StateManager.prototype.setupResultRanking = function () {
        ...
        lengthRankingList.forEach(function (p) { return lengthRankingPlayerIdList.push({
            playerId: p.player.id,
            count: _this.playerList[p.player.id].lengthCount
        }); });
        killRankingList.forEach(function (p) { return killRankingPlayerIdList.push({
            playerId: p.player.id,
    ...
    StateManager.prototype.dividePlayerCountIntoTiers = function () {
        ...
        Object.keys(this.playerList).forEach(function (playerId) {
            var playerState = _this.playerList[playerId].state;
            if (StateRoleChecker_1.checkStateRole(playerState, StateRoleChecker_1.StateRoleType.CanCountType))
                ++playerCount;
        });
        if (playerCount <= 10) {
            return 4;
    ...
    StateManager.prototype._generateNewSnakeLayout = function (playerId, snakeLayer, rebornEffect, nowRadius) {
        ...
        var maxLength = (nowRadius - snakeConfig.baseSpeed *
...
var PlayerInfo = /** @class */ (function () {
    function PlayerInfo(param) {
        ...
        this.respawnTimes = (param.user.isPremium || param.isBroadcaster) ?
            defaultParameter_1.sessionParameter.config.snake.premiumRespawnTimes :
            defaultParameter_1.sessionParameter.config.snake.respawnTimes;
        this.killCount = 0;
    ...
    PlayerInfo.prototype.getsLastWords = function () {
        return this.snake.words.reduce(function (str, ch) { return str + ch; }, "");
    };
    return PlayerInfo;
}());
